...
function App() {
  ...
  const selectedBtnHandleClick = (item, credit, price) => {
    ...
    if (!isExist && credit <= remaining) {
      ...
      alert('Sorry!! An item cannot be added to the cart section twice and above 20 credits.');
    }
  };
  ...
  return (
    <>
      <Header />  
      <div className="md:flex md:mx-40 justify-center">
        <Items selectedBtnHandleClick={selectedBtnHandleClick} /> 
        <Carts selectedItems={selectedItems} totalcredit={totalcredit} cardTotalPrice={cardTotalPrice} remaining={remaining} /> 
      </div>
